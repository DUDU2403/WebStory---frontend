...
import { Eye, EyeOff, ArrowLeft, Key, Store, Sun, Moon } from 'lucide-react';
...
export default function RegisterVendedor({ nav }) {
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      await lojaRegister(form);
      show('Loja criada! Faça login.', 'success');
      nav('login-vendedor');
    } catch (e) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--surface-2)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, position: 'relative' }}>
      ...
      <div className="card animate-slideUp" style={{ width: '100%', maxWidth: 460, padding: 40 }}>
        <button className="btn btn-ghost btn-sm" onClick={() => nav('login-vendedor')} style={{ marginBottom: 24, paddingLeft: 0 }}>
        ...
        {step === 1 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {[
              { label: 'Nome da loja *', key: 'nome', ph: 'Ex: Peças & Cia', type: 'text' },
              { label: 'E-mail *', key: 'email', ph: 'contato@loja.com', type: 'email' },
              { label: 'Telefone / WhatsApp *', key: 'telefone', ph: '(11) 99999-9999', type: 'text' },
            ].map(f => (
              <div key={f.key}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input className="input" type={showPass ? 'text' : 'password'} placeholder="Mínimo 6 caracteres"
                  value={form.senha} onChange={e => set('senha', e.target.value)} style={{ paddingRight: 44 }} />
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 10, padding: '12px 14px', display: 'flex', gap: 10 }}>
              <Key size={15} color="#d97706" style={{ marginTop: 2, flexShrink: 0 }} />
              <p style={{ fontSize: 13, color: '#92400e', lineHeight: 1.6 }}>
                Você precisa de uma <strong>chave de acesso</strong> fornecida pelo administrador.
              </p>
            </div>
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', display: 'block', marginBottom: 5 }}>Chave de acesso *</label>
              <input className="input" placeholder="AK-XXXXXXXX" value={form.chaveAcesso}
                onChange={e => set('chaveAcesso', e.target.value.toUpperCase())}
                style={{ fontFamily: 'monospace', letterSpacing: '0.05em' }} />
            </div>
            <div>
              <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', display: 'block', marginBottom: 5 }}>CNPJ <span style={{ fontWeight: 400, color: 'var(--text-3)' }}>(opcional)</span></label>
